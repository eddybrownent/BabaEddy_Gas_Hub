import {
  View, Text, StatusBar, ScrollView, Image, TouchableOpacity,
} from 'react-native';
...
import { addToCart, removeFromCart, selectCartproductsById } from '../slices/cartSlice';
...
function ProductsScreen() {
  ...
  const handleIncrease = (productId, name, price, image, description) => {
    dispatch(addToCart({
      id: productId, name, price, image, description,
    }));
  };
  ...
  return (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 50 }}>
            ...
            <View style={{
              borderTopLeftRadius: 40, borderTopRightRadius: 40, backgroundColor: 'white', marginTop: -60, paddingTop: 20,
            }}>
                    ...
                    {products.map((product) => (
                        <View key={product.id}
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                >{getItemQuantityInCart(product.id)}</Text>
                                <TouchableOpacity
